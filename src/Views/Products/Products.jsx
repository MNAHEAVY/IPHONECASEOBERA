...
import Loader from "../../Components/Loader/Loader";
const sortOptions = [
  { name: "Mas Populares", value: "popular" },
  { name: "Nuevos", value: "newest" },
...
function getMinVariantPrice(product) {
  if (product?.displayPrice) return Number(product.displayPrice) || 0;
...
function hasVariantAttribute(product, attrName, selectedValues) {
  if (!selectedValues.length) return true;

  return product?.variants?.some((variant) => {
    const value = normalizeText(variant?.attributes?.[attrName]);
...
function buildDynamicFilters(products) {
  ...
  products.forEach((product) => {
    (product?.variants || []).forEach((variant) => {
      ...
      const model = variant?.attributes?.model;

      if (color) {
        colorsMap.set(normalizeText(color), formatLabel(color));
      }

      if (storage) {
        storageMap.set(normalizeText(storage), formatLabel(storage));
      }

      if (model) {
        modelMap.set(normalizeText(model), formatLabel(model));
      }
    });
  });
...
export default function Products() {
  ...
  const productsWithComputedData = useMemo(() => {
  ...
  const dynamicFilters = useMemo(() => {
    return buildDynamicFilters(productsWithComputedData);
  }, [productsWithComputedData]);

  const filteredProducts = useMemo(() => {
    const normalizedQuery = normalizeText(query);

    const result = productsWithComputedData.filter((product) => {
      const matchesQuery =
        !normalizedQuery ||
        normalizeText(product?.name).includes(normalizedQuery) ||
        normalizeText(product?.subCategory).includes(normalizedQuery) ||
        normalizeText(product?.category).includes(normalizedQuery) ||
        normalizeText(product?.description).includes(normalizedQuery);

      const matchesColor = hasVariantAttribute(product, "color", selectedFilters.color);

      const matchesStorage = hasVariantAttribute(
        product,
        "storage",
        selectedFilters.almacenamiento,
      );

      const matchesModel = hasVariantAttribute(product, "model", selectedFilters.modelo);
    ...
    return result;
  }, [productsWithComputedData, query, selectedFilters, selectedSort]);

  const renderFilterInput = (section, option, optionIdx, mobile = false) => {
    const id = mobile
      ? `filter-mobile-${section.id}-${optionIdx}`
    ...
    return (
      <div key={option.value} className='flex items-center'>
        <input
          ...
          className='h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500'
        />
        <label htmlFor={id} className='ml-3 min-w-0 flex-1 text-sm text-gray-600'>
          {option.label}
        </label>
      </div>
    );
  ...
  return (
    <div>
              ...
              {dynamicFilters.map((section) => (
                ...
                >
                  ...
                  <DisclosurePanel className='pt-6'>
                    <div className='space-y-6'>
                      {section.options.map((option, optionIdx) =>
                        renderFilterInput(section, option, optionIdx, true),
                      )}
                    </div>
                  </DisclosurePanel>
                </Disclosure>
      ...
      <main className="mx-auto bg-slate-50 bg-[url('/src/assets/beams-components.png')] px-4 sm:px-6 lg:px-8">
        <div className='flex items-baseline justify-between border-b border-gray-200 pb-6 pt-24'>
          <h1 className='text-4xl font-bold tracking-tight text-gray-900'>
            Tus Productos{query ? `: ${query}` : ""}
          </h1>
        ...
        <section aria-labelledby='products-heading' className='pb-24 pt-6'>
          ...
          <div className='grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4'>
              ...
              {dynamicFilters.map((section) => (
                ...
                >
                  ...
                  <DisclosurePanel className='pt-6'>
                    <div className='space-y-4'>
                      {section.options.map((option, optionIdx) =>
                        renderFilterInput(section, option, optionIdx),
                      )}
                    </div>
                  </DisclosurePanel>
                </Disclosure>
            ...
            <div className='lg:col-span-3'>
              <div className='bg-slate-50/[.06]'>
                <div className='mx-auto max-w-2xl px-4 py-16 pb-24 sm:px-6 sm:pt-4 lg:max-w-7xl lg:px-8'>
                  <h2 className='sr-only'>Products</h2>

                  <div className='overflow-y-auto max-h-[600px]'>
                    <div className='z-1 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8'>
                      {!prod ? (
                        <Loader />
                      ) : filteredProducts.length === 0 ? (
                        ...
                        filteredProducts.map((product) => (
                          ...
                          >
                            ...
                            {!product.available && (
                              <p className='mt-1 text-sm text-red-500'>Sin stock</p>
                            )}
                          </Link>
                        ))
                      )}
